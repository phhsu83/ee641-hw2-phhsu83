...
from training_utils import kl_annealing_schedule, temperature_annealing_schedule, sample_diverse_patterns
from analyze_latent import interpolate_styles, style_transfer_example, visualize_latent_hierarchy, measure_disentanglement, check_musically_valid
...
def compute_hierarchical_elbo(
    model,
    x,                  # [B, 16, 9], float in {0,1}
    recon_logits,       # [B, 16, 9], decoder 輸出 logits（未 sigmoid）
    mu_low, logvar_low, # [B, D_low]
    mu_high, logvar_high,# [B, D_high]
    z_high,             # [B, D_high]  ← 用 sample，而不是 mu_high
    beta=1.0,
    free_bits=0.5,     # e.g., 0.5 (nats/維); 若 None 表示不用 FB
):
    # 1) Reconstruction
    recon_loss = F.binary_cross_entropy_with_logits(
        recon_logits, x.float(), reduction="mean"
    )

    # 2) KL_high: q(z_high|·) || N(0,I)
    # per-dim kl: [B, D_high]
    var_high = torch.exp(logvar_high)
    kl_high_per = 0.5 * (mu_high.pow(2) + var_high - logvar_high - 1.0)

    # 先對 batch 平均，得到 [D_high]
    kl_high_dim = kl_high_per.mean(dim=0)
    # Free bits（可選）
    if free_bits is not None:
        kl_high_dim = torch.clamp(kl_high_dim - free_bits, min=0.0)
    kl_high = kl_high_dim.sum()


    # 3) KL_low: q(z_low|x) || p(z_low|z_high)
    # 由 z_high 計條件先驗
    mu_p, logvar_p = torch.chunk(model.prior_low(z_high), 2, dim=-1)  # 用 z_high，而非 mu_high
    var_q = torch.exp(logvar_low)
    var_p = torch.exp(logvar_p)
    kl_low_per = 0.5 * ((logvar_p - logvar_low) + (var_q + (mu_low - mu_p).pow(2)) / var_p - 1.0)

    kl_low_dim = kl_low_per.mean(dim=0)
    if free_bits is not None:
        kl_low_dim = torch.clamp(kl_low_dim - free_bits, min=0.0)
    kl_low = kl_low_dim.sum()
...
def train_epoch(model, data_loader, optimizer, epoch, device, config):
    ...
    beta = kl_annealing_schedule(epoch, method=config['kl_anneal_method'], total_epochs=config["num_epochs"], cycles=4)
    # temperature = temperature_annealing_schedule(epoch, method="linear", total_epochs=config["num_epochs"], start=1.0, end=0.5)
    temperature = 1.0
    ...
    for batch_idx, (patterns, styles, densities) in enumerate(data_loader):
        ...
        loss, recon_loss, kl_low, kl_high = compute_hierarchical_elbo(
            model,
            patterns,                  # [B, 16, 9], float in {0,1}
            recon,       # [B, 16, 9], decoder 輸出 logits（未 sigmoid）
            mu_low, logvar_low, # [B, D_low]
            mu_high, logvar_high,# [B, D_high]
            mu_high,             # [B, D_high]  ← 用 sample，而不是 mu_high
            beta=beta,
            free_bits=0.5,     # e.g., 0.5 (nats/維); 若 None 表示不用 FB
        )
...
def main():
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                for patterns, styles, densities in val_loader:
                    ...
                    recon, mu_low, logvar_low, mu_high, logvar_high = model(patterns, beta=1.0, temperature=1.0)
                    # loss, recon_loss, kl_low, kl_high = compute_hierarchical_elbo(
                    ...
                    loss, recon_loss, kl_low, kl_high = compute_hierarchical_elbo(
                        ...
                        mu_high,             # [B, D_high]  ← 用 sample，而不是 mu_high
                        beta=1.0,
                        free_bits=None,     # e.g., 0.5 (nats/維); 若 None 表示不用 FB
                    )
    ...
    #
    sample_diverse_patterns(model, n_styles=5, n_variations=10,
                            temperature=1.0, threshold=0.5,
                            device=config["device"])
    
    #
    pattern1 = val_dataset[0][0].unsqueeze(0).to(config['device'])
    pattern1_style = val_dataset[0][1]
    pattern2 = val_dataset[1][0].unsqueeze(0).to(config['device'])
    pattern2_style = val_dataset[1][1]
    interpolate_styles(model, pattern1, pattern2, n_steps=10, 
                        device=config["device"], 
                        temperature=1.0, threshold=0.5)
    
    #
    style_transfer_example(model, pattern1, pattern2, pattern1_style, pattern2_style, 
                           device=config["device"], threshold=0.5)
    
    #
    visualize_latent_hierarchy(model, val_loader, device=config["device"])

    #
    measure_disentanglement(model, val_loader, device=config["device"])

    #
    musical_score = check_musically_valid(model, val_loader, device=config["device"], threshold= 0.5)
    print("Musical Score:", musical_score)
    ...
    history["config"]["device"] = str(history["config"]["device"])
