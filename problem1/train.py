...
def main():
    ...
    fig = interpolation_experiment(generator, config['device'])
    fig.savefig(f"results/visualizations/interpolation_sequences_{config['experiment']}.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    
    # Save results
    # TODO: Save training history to JSON
    with open(f"{config['results_dir']}/training_log_{config['experiment']}.json", 'w') as f:
    ...
    }, f"{config['results_dir']}/best_generator_{config['experiment']}.pth")
